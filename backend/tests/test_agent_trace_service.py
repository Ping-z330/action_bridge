...
def test_create_agent_trace_log_records_tool_metadata(db_session) -> None:
    create_agent_trace_log(
        ...
        intent=AgentIntent(name="query_tasks", filters={"status": "open"}),
        tool_executed=True,
        response=AgentResponse(handled=True, message="找到 1 个任务"),
    )
