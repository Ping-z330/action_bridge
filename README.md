...
17. 运行 `/api/follow-ups/run` 或开启自动跟进，收到提醒后回复“完成了 #任务ID”，验证任务状态自动更新。
...
ActionBridge 是一个会议执行闭环 Agent MVP，基于 FastAPI、Next.js、SQLite、DeepSeek API 和飞书机器人集成，实现会议纪要结构化解析、行动项生成、任务状态跟进、历史归档、飞书消息接入、上下文感知回复、自然语言任务创建确认、截止时间修改确认、负责人修改确认、LLM 意图识别兜底、任务查询/更新、项目进度总结、自动提醒和提醒回复状态更新，帮助团队降低会议后整理与执行跟进成本。
